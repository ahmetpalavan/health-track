...
export const updateAppointment = async ({ appointmentId, userId, timeZone, appointment, type }: UpdateAppointmentParams) => {
  try {
    ...
    const smsMessage = `Greetings from Healthcrack. ${
      type === 'schedule'
...
export const getAppointment = async (userId: string) => {
  try {
    const appointment = await databases.getDocument(DATABASE_ID!, APPOINTMENT_COLLECTION_ID!, userId);
...
export const sendSMSNotification = async (userId: string, content: string) => {
  try {
    ...
    console.error('An error occurred while sending the SMS notification:', error);
  }
};
